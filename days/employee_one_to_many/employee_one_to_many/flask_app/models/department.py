...
class Department:
    ...
    def get_all_departments(cls):
        ...
        for row in results:
            ...
            if len(departments) == 0:
                ...
                new_department = cls(row)
                departments.append(new_department)
            # now determine if employee exists and, if exists, create a new
            # Employee instance and associate the two
    ...
    def get_single_department(cls, data):
        ...
        for row in results:
            if row['employees.id'] != None:
                ...
                new_employee = Employee(new_employee_data)
                single_department.employees.append(new_employee)
                new_employee.department = single_department
    ...
    def validate_department(data):
        ...
        if len(data['name']) > 100:
            is_valid = False
            flash("Department name must be 100 characters or less.")

        if len(data['name']) < 2:
            is_valid = False
            flash("Department name must be 2 characters or greater.")

        if len(data['location']) > 50:
            is_valid = False
            flash("Department location must be less than 50 characters.")

        if len(data['location']) < 3:
            is_valid = False
            flash("Department location must be 3 or more characters.")
